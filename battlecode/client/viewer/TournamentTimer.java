...
public class TournamentTimer {
	...
	private Thread thread = new Thread() {
		...
		public void run() {
			...
			while (true) {
				...
				if (minimap != null) { minimap.setTimeline(gst); }
				if(minimap != null) {
					if(aWins > 0) minimap.addWin(Team.A);
					if(bWins > 0) minimap.addWin(Team.B);
				}
				br.beginIntroCutScene(loop.stop());
				while (loop.isPlaying()) { doSleep(100); }
				doSleep(2000);
				br.setCutSceneVisible(false);
				new MatchPlayer(null, controller, gst, null, false);
				while (gst.getRound() < gst.getNumRounds()) { doSleep(100); }
				doSleep(1000);
	...
	public Runnable getSpaceBarListener() { return spaceBarListener; }
}
