...
public class GameSoundBank {
    ...
    public static class ClipGroup {
        ...
        public ClipGroup(String path, long repeat, int maxClips, boolean
                active) {
...
//			clips = new LinkedList<Clip>();
        ...
        public synchronized void play() {
            ...
            //System.out.println(now + " " + (lastPlayed + 1000*repeat -
            // fadeTime*1000));
            if (now < lastPlayed + 1000 * repeat) {
                /*if(ambientMusic && now > lastPlayed + 1000*repeat - ((long)
				fadeTime*1000)){
					...
					delta /= 100.0f;
					level.shift(currentValue, currentValue-delta, fadeTime);
					// linshift
                ...
                //The level.shift implementation does not currently do a
                // smooth shift.
                return;
            }
...
//			if(!active) return;
			...
			if (clips.size() == maxClips) {
				level = getGainControl(clips.getFirst());
				level.shift(level.getValue(), level.getMinimum(), fadeTime);
				// linshift
			}
			*/
        }
    ...
    public static ClipGroup DEATH;// = new ClipGroup("sound/explosion.wav",
    // 450000, 3);
    ...
    public static ClipGroup SNIPE;//  = new ClipGroup("sound/snipe.wav",
    // 250000);

    public static ClipGroup FLOP;// = new ClipGroup("sound/flop.wav");
    public static ClipGroup GLOP;// = new ClipGroup("sound/glop.wav");
    public static ClipGroup CAPTURE;//= new ClipGroup("sound/laser_rocket
    // .wav", 500000, 2);
    ...
    //public static final ClipGroup FLUX_BURN = new ClipGroup
    // ("sound/flux_burn.wav");
