...
class EMPDetonationAnim extends Animation {
    ...
    public void draw(Graphics2D g2) {
        g2.setColor(new Color(1 - roundAge() * dColor, 1, 1, 1 - roundAge() *
                dColor));
        float r = 25 *
                (1 - (float) Math.pow((double) (maxFrame - curFrame) /
                        maxFrame, 3.5));
        g2.fill(new Ellipse2D.Float(loc.x + 0.5f - r, loc.y + 0.5f - r, 2 *
                r, 2 * r));
    }
